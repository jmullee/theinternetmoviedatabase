...
int readFilmographyToListRec(FILE *dbFp, long offset, long nameKey, struct listRec *lrec, int listId)
    ...
    lrec->count = count;
    (void)qsort((void *)lrec->entries, (size_t)count, sizeof(struct listEntry),
                (int (*)(const void *, const void *))listRecTitleKeySort);
    return (TRUE);
    }
        ...
        for (listId = 0; listId < NO_OF_FILMOGRAPHY_LISTS; listId++)
            if (nrec->lists[listId] != NULL)
                for (i = 0; i < nrec->lists[listId]->count; i++)
                    {
                    nrec->lists[listId]->entries[i].title =
                        mapTitleKeyToText(nrec->lists[listId]->entries[i].titleKey, keyFp, indexFp);
    ...
    for (nrec = chain; nrec != NULL; nrec = nrec->next)
        ...
        for (listId = 0; listId < NO_OF_FILMOGRAPHY_LISTS; listId++)
            if (nrec->lists[listId] != NULL)
                for (i = 0; i < nrec->lists[listId]->count; i++)
                    {
                    nrec->lists[listId]->entries[i].attr =
                        mapAttrKeyToText(nrec->lists[listId]->entries[i].attrKey, keyFp, indexFp);
...
int yearNameSearchSort(struct listEntry *e1, struct listEntry *e2) { return (e1->year - e2->year); }
            ...
            for (i = nrec->firstMatch; i < NO_OF_FILMOGRAPHY_LISTS; i++)
                if (nrec->lists[i] != NULL && nrec->lists[i]->count > 0)
                    ...
                    if (nrec->searchparams.mrropt == SMRR)
                        (void)qsort((void *)nrec->lists[i]->entries, (size_t)nrec->lists[i]->count,
                                    sizeof(struct listEntry), (int (*)(const void *, const void *))smrrNameSearchSort);
                    else if (nrec->searchparams.mrropt == VMRR)
                        (void)qsort((void *)nrec->lists[i]->entries, (size_t)nrec->lists[i]->count,
                                    sizeof(struct listEntry), (int (*)(const void *, const void *))vmrrNameSearchSort);
                    else if (nrec->searchparams.yropt == YR)
                        (void)qsort((void *)nrec->lists[i]->entries, (size_t)nrec->lists[i]->count,
                                    sizeof(struct listEntry), (int (*)(const void *, const void *))yearNameSearchSort);
                    else
                        (void)qsort((void *)nrec->lists[i]->entries, (size_t)nrec->lists[i]->count,
                                    sizeof(struct listEntry), (int (*)(const void *, const void *))alphaNameSearchSort);
                    }
    }
        ...
        if (minRec->lists[k] != NULL)
            for (i = 0; i < minRec->lists[k]->count; i++)
                ...
                if (matched)
                    ...
                    if (matched)
                        {
                        returnrec->lists[0]->entries[count].title =
                            duplicateString(minRec->lists[minRec->firstMatch]->entries[i].title);
                        ...
                        returnrec->lists[0]->entries[count].mrr =
                            cloneMrrData(minRec->lists[minRec->firstMatch]->entries[i].mrr);
                        returnrec->lists[0]->entries[count].aka =
                            cloneAkaData(minRec->lists[minRec->firstMatch]->entries[i].aka);
                ...
                for (i = 0; i < nrec->lists[listId]->count; i++)
                    ...
                    for (quitLoop = FALSE, j = 0; !quitLoop && j < NO_OF_TITLE_INFO_LISTS; j++)
                        ...
                        if (constraints->titleInfoString[j][0])
                            if (constrainTitleInfo(titleInfoDb[j], titleInfoIdx[j], titleKey,
                                                   constraints->titleInfoString[j]) == FALSE)
                                quitLoop = TRUE;
                        }
                    if (quitLoop)
