...
int getNextLindexTitle(FILE *dbFp, FILE *indexFp, FILE *keyFp, struct searchConstraints *constraints, char *title,
                       TitleID *titleKey, int *yearVal)
    {
    int stopFlag = FALSE;
    long offset;
...
void lindexTitles(int yropt, int mrropt, struct searchConstraints *constraints)
        ...
        for (quitLoop = FALSE, i = 0; !quitLoop && i < NO_OF_TITLE_INFO_LISTS; i++)
            ...
            if (constraints->titleInfoString[i][0])
                ...
                if (titleInfoKey[i] != titleKey)
                    ...
                    {
                    if (checkConstraints(titleInfoDb[i], titleInfoOffset[i], titleKey,
                                         constraints->titleInfoString[i]) == FALSE)
                        quitLoop = TRUE;
                    }
                }
            }
    ...
    if (yropt == YR || mrropt == SMRR || mrropt == VMRR)
        {
        if (yropt == YR)
            qsort((void *)array, (size_t)count, sizeof(struct lindexTitleRec),
                  (int (*)(const void *, const void *))yearLindexSort);
        else if (mrropt == SMRR)
            qsort((void *)array, (size_t)count, sizeof(struct lindexTitleRec),
                  (int (*)(const void *, const void *))smrrLindexSort);
        else
            qsort((void *)array, (size_t)count, sizeof(struct lindexTitleRec),
                  (int (*)(const void *, const void *))vmrrLindexSort);
        ...
        for (i = 0; i < count; i++)
            ...
            if (yropt == YR && mrropt == NONE)
                ...
                if (array[i].mrr.votes == 0)
                    ...
                    {
                    (void)printf("      %s%8d   %2.1f  ", array[i].mrr.distribution, array[i].mrr.votes,
                                 array[i].mrr.rating);
                    displayTitleYear(array[i].title, array[i].year);
                    (void)printf("\n");
                    }
                ...
                if (array[i].mrr.votes == 0)
                    ...
                    (void)printf("      %s%8d   %2.1f  %s\n", array[i].mrr.distribution, array[i].mrr.votes,
                                 array[i].mrr.rating, array[i].title);
                }
