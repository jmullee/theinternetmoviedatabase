...
int mrrKeyLformatCompare(struct mrrDbRec *search, struct mrrDbRec *m1) { return (search->titleKey - m1->titleKey); }

void sortLformatData(struct formatRec listData[], int count, int raw, int mrropt, int yropt, struct titleDbRec *years,
    ...
    if (mrropt == NONE && yropt == NONE && !raw)
        ...
        qsort((void *)listData, (size_t)count, sizeof(struct formatRec),
              (int (*)(const void *, const void *))plainAlphaSort);
        return;
        }
    ...
    if (yropt != NONE)
        ...
        for (i = 0; i < count; i++)
            ...
            if ((trec = bsearch((void *)&tSearchRec, (void *)years, (size_t)nyears, sizeof(struct titleDbRec),
                                (int (*)(const void *, const void *))titleKeyLformatCompare)) != NULL)
    ...
    if (mrropt != NONE)
        ...
        for (i = 0; i < count; i++)
            ...
            if ((mrec = bsearch((void *)&mSearchRec, (void *)ratings, (size_t)nratings, sizeof(struct mrrDbRec),
                                (int (*)(const void *, const void *))mrrKeyLformatCompare)) != NULL)
...
void lformatList(int listId, int mvsonly, int raw, int mrropt, int yropt, struct titleDbRec *years, TitleID nyears,
                 struct mrrDbRec *ratings, TitleID nratings)
    {
