...
void displayAttributes(char *attr, char *cname, int chopt)
    ...
    if (attr == NULL)
        if (cname == NULL || chopt == FALSE)
            ...
            (void)printf("  [%s]\n", cname);
    else if (cname == NULL || chopt == FALSE)
        (void)printf(" %s\n", attr);
    else
        (void)printf(" %s  [%s]\n", attr, cname);
    }
            ...
            if (!mvsonly || lrec[i].title[0] != '"')
                ...
                if (mrropt)
                    if (lrec[i].mrr != NULL && lrec[i].mrr->votes > 0)
                        (void)printf("      %s%8d   %2.1f  ", lrec[i].mrr->distribution, lrec[i].mrr->votes,
                                     lrec[i].mrr->rating);
                    else
                        (void)printf("                                ");
                else if (tidy)
            ...
            if (tidy)
                ...
                for (i = 0; i < NO_OF_FILMOGRAPHY_LISTS; i++)
                    if (nrec->lists[i] != NULL && nrec->lists[i]->count > 0)
                        ...
                        displayTitleAttrPairs(nrec->lists[i]->entries, nrec->lists[i]->count, tidy,
                                              nrec->searchparams.mrropt, nrec->searchparams.mvsonly,
...
void displayTidyFilmographyListData(struct titleListEntry *lrec)
    ...
    if ((comma = strchr(tmp, ',')) != NULL)
        {
        if (*(comma + strlen(comma) - 1) == ')' && (bracket = strstr(comma, " (")) != NULL)
            {
            *comma = '\0';
            *bracket = '\0';
            if (lrec->attr != NULL)
    ...
    for (i = 0; i < NO_OF_FILMOGRAPHY_LISTS - 3; i++)
        ...
        if (count > 0)
            ...
            if (count == 1)
                ...
                (void)qsort((void *)&(trec->entries[base]), (size_t)count, sizeof(struct titleListEntry),
...
void displayTitleSearchRec(struct titleSearchRec *trec, int tidy)
    ...
    if (tidy)
        ...
        if (trec->mrr != NULL && trec->mrr->votes > 0)
            (void)printf("      %s%8d   %2.1f  ", trec->mrr->distribution, trec->mrr->votes, trec->mrr->rating);
        else
            (void)printf("                                ");
