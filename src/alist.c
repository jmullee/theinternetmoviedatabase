...
void displayAlistSorted(struct alistResult *results, int count)
    ...
    qsort((void *)results, (size_t)count, sizeof(struct alistResult),
          (int (*)(const void *, const void *))alistYearSort);
...
int alistAttrTest(struct attrSearchOptRec attrFlags, char *attr, int substring, int casesen, char *listAttr)
    ...
    if (attrFlags.string)
        ...
        if (substring)
            ...
            else if (caseStrStr(listAttr, attr) != NULL)
                return (TRUE);
            }
        else
            {
...
int titleKeyAlistCompare(struct titleDbRec *search, struct titleDbRec *t1) { return (search->titleKey - t1->titleKey); }

int alistYearTest(int yearFlag, int yrf, int yrt, TitleID titleKey, TitleID nyears, struct titleDbRec *years,
                  int *yearValue)
    {
    ...
    if (yearFlag)
        ...
        if ((trec = bsearch((void *)&searchRec, (void *)years, (size_t)nyears, sizeof(struct titleDbRec),
                            (int (*)(const void *, const void *))titleKeyAlistCompare)) != NULL)
            *yearValue = trec->year;
        else
            return (FALSE);
        if (*yearValue < yrf)
...
void displayAlistTitleSorted(struct alistTitleRes results[], int count)
    ...
    qsort((void *)results, (size_t)count, sizeof(struct alistTitleRes),
          (int (*)(const void *, const void *))alistTitleYearSort);
...
void alistTitles(char *attr, int substring, int casesen, struct attrSearchOptRec attrFlags, int mvsonly, int yr,
                 int yrf, int yrt)
    {
