...
void displayLseenRatings(struct formatRec listData[], int count)
    ...
    for (i = 0; i < count; i++)
        ...
        if (listData[i].mrr.votes > 0)
            (void)printf("      %s%8d   %2.1f  ", listData[i].mrr.distribution, listData[i].mrr.votes,
                         listData[i].mrr.rating);
        else
            (void)printf("                                ");
...
int mrrTitleKeyLseenCompare(struct mrrDbRec *search, struct mrrDbRec *m1) { return (search->titleKey - m1->titleKey); }
...
void mapLseenData(struct formatRec listData[], int count, FILE *titleKeyFp, FILE *titleIndexFp, FILE *attrKeyFp,
                  FILE *attrIndexFp)
    {
    int i;
...
void sortLseenData(struct formatRec listData[], int count, int mrropt, int yropt, struct titleDbRec *years,
                   TitleID nyears, struct mrrDbRec *ratings, TitleID nratings)
    {
    ...
    if (yropt != NONE)
        ...
        for (i = 0; i < count; i++)
            ...
            if ((trec = bsearch((void *)&tSearchRec, (void *)years, (size_t)nyears, sizeof(struct titleDbRec),
                                (int (*)(const void *, const void *))yearTitleKeyLseenCompare)) != NULL)
    ...
    if (mrropt != NONE)
        ...
        for (i = 0; i < count; i++)
            ...
            if ((mrec = bsearch((void *)&mSearchRec, (void *)ratings, (size_t)nratings, sizeof(struct mrrDbRec),
                                (int (*)(const void *, const void *))mrrTitleKeyLseenCompare)) != NULL)
    ...
    if (mrropt == NONE && yropt == YR)
        ...
        qsort((void *)listData, (size_t)count, sizeof(struct formatRec),
              (int (*)(const void *, const void *))smrrLformatSort);
    else if (mrropt == VMRR)
        qsort((void *)listData, (size_t)count, sizeof(struct formatRec),
              (int (*)(const void *, const void *))vmrrLformatSort);
    }
...
int readLseenData(struct formatRec listData[], struct voteDbRec *votes, TitleID nvotes, FILE *dbFp, int listId)
    ...
    for (i = 0; i < noWithAttr; i++)
        ...
        if (bsearch((void *)&searchRec, (void *)votes, (size_t)nvotes, sizeof(struct voteDbRec),
                    (int (*)(const void *, const void *))voteTitleKeyLseenCompare) != NULL)
            count++;
        }
...
int lseenCountSort(struct lseenCount *r1, struct lseenCount *r2) { return (r2->count - r1->count); }

void displayLseenCounts(struct lseenCount counts[], int ncount, int mrropt, int yropt, struct voteDbRec *votes,
                        TitleID nvotes, struct titleDbRec *years, TitleID nyears, struct mrrDbRec *ratings,
                        TitleID nratings, FILE *dbFp, int listId)
    {
    ...
    qsort((void *)counts, (size_t)ncount, sizeof(struct lseenCount),
          (int (*)(const void *, const void *))lseenCountSort);
    for (i = 0; i < ncount; i++)
        {
        if (mrropt != NONE || yropt != NONE)
            {
...
void lseenList(int listId, int mrropt, int yropt, int usopt, int threshold, struct voteDbRec *votes, TitleID nvotes,
               struct titleDbRec *years, TitleID nyears, struct mrrDbRec *ratings, TitleID nratings)
    {
    FILE *dbFp, *nameKeyFp, *nameIndexFp, *titleKeyFp = NULL, *titleIndexFp = NULL, *attrKeyFp = NULL,
                                          *attrIndexFp = NULL;
    struct formatRec listData[MAXFILMOGRAPHIES];
    ...
    else if ((counts = calloc(MAXLSEEN, sizeof(struct lseenCount))) == NULL)
    ...
    if (!usopt)
        displayLseenCounts(counts, ncount, mrropt, yropt, votes, nvotes, years, nyears, ratings, nratings, dbFp,
                           listId);
