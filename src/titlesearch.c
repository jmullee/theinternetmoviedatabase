...
void findTitleData(FILE *dbFp, long offset, TitleID titleKey, int dupeCount, NameID *nameKey, AttributeID *attrKey,
                   char **cname, int *position, int *lineOrder, int *groupOrder, int *subgroupOrder, int listId)
    {
    ...
    if (trec->titleKey != NOTITLE)
        if (findTitleIndexBase(indexFp, trec->titleKey))
            ...
            while (titleKey == trec->titleKey)
                {
                findTitleData(dbFp, offset, titleKey, dupeCount, &nameKey, &attrKey, &cname, &position, &lineOrder,
                              &groupOrder, &subgroupOrder, listId);
...
struct titleSearchRec *makeTitleChain(struct nameSearchRec *nrec, struct titleSearchOptRec options, int trivopts[],
                                      struct nameSearchOptRec noptions, int titleInfoOpts[])
    {
