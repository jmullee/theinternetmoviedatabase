...
TitleID readMoviesList(struct titleIndexRec *titles)
    ...
    (void)qsort((void *)titles, (size_t)indexKey, sizeof(struct titleIndexRec),
                (int (*)(const void *, const void *))titleIndexRecSort);

    return (indexKey);
    }

int nameKeyOffsetSort(struct nameKeyOffset *n1, struct nameKeyOffset *n2) { return (n1->nameKey - n2->nameKey); }

int titleKeyOffsetSort(struct titleKeyOffset *t1, struct titleKeyOffset *t2) { return (t1->titleKey - t2->titleKey); }

int attrKeyOffsetSort(struct attrKeyOffset *a1, struct attrKeyOffset *a2) { return (a1->attrKey - a2->attrKey); }
...
void writeTitleIndexKey(TitleID titleCount)
    ...
    (void)qsort((void *)titleIndex, (size_t)count, sizeof(struct titleKeyOffset),
                (int (*)(const void *, const void *))titleKeyOffsetSort);
...
void writeAttrIndexKey(AttributeID attrCount)
    ...
    (void)qsort((void *)attrIndex, (size_t)count, sizeof(struct attrKeyOffset),
                (int (*)(const void *, const void *))attrKeyOffsetSort);
...
struct attrIndexRec *findAttrKeyPos(char *attr, struct attrIndexRec *attributes, AttributeID attrCount,
                                    AttributeID *insert)
    {
...
int yearDataSort(struct yearData *y1, struct yearData *y2) { return (y1->titleKey - y2->titleKey); }
...
TitleID processMoviesList(struct titleIndexRec *titles, TitleID *titleCount, AttributeID *attrCount)
    ...
    (void)qsort((void *)years, (size_t)count, sizeof(struct yearData),
                (int (*)(const void *, const void *))yearDataSort);
...
char *splitChar(char *line)
    ...
    if ((ptr = strchr(line, '[')) == NULL)
        return (NULL);
    else if ((endptr = strrchr(ptr, ']')) == NULL)
        return (NULL);
...
void makeCastDatabaseTitlesIndex(int listId, long nentries)
    ...
    (void)qsort((void *)titlesIndex, (size_t)count, sizeof(struct titleKeyOffset),
                (int (*)(const void *, const void *))titleKeyOffsetSort);
...
long processCastList(NameID *nameCount, struct titleIndexRec *titles, TitleID *titleCount, AttributeID *attrCount,
                     int listId, int moviesOnly, int nochar)
    {
                    ...
                    *title++ = '\0';
                    ...
                    if ((!skipMode) && ((!moviesOnly) || (moviesOnly && *title != '"')))
                        ...
                        if (attr == NULL || currentEntry.noWithAttr >= MAXWITHATTRS)
                            {
                            currentEntry.withoutAttrs[currentEntry.noWithoutAttr].titleKey =
                                titleKeyLookup(title, titles, titleCount);
                            ...
                            {
                            currentEntry.withAttrs[currentEntry.noWithAttr].titleKey =
                                titleKeyLookup(title, titles, titleCount);
...
long processFilmographyList(NameID *nameCount, struct titleIndexRec *titles, TitleID *titleCount,
                            AttributeID *attrCount, int listId, int moviesOnly)
    {
    FILE *listFp, *dbFp, *nameKeyFp, *tmpFp;
    char line[MXLINELEN];
    char keyFileData[MXLINELEN];
                ...
                if ((title = strchr(line, '\t')) != NULL)
                    ...
                    *title++ = '\0';
                    ...
                    if ((!skipMode) && ((!moviesOnly) || (moviesOnly && *title != '"')))
                        ...
                        if (attr == NULL || currentEntry.noWithAttr >= MAXWITHATTRS)
                            {
                            currentEntry.withoutAttrs[currentEntry.noWithoutAttr] =
                                titleKeyLookup(title, titles, titleCount);
                            currentEntry.noWithoutAttr++;
                            }
                        else
...
long processWriterFilmographyList(NameID *nameCount, struct titleIndexRec *titles, TitleID *titleCount,
                                  AttributeID *attrCount, int listId, int moviesOnly)
    {
    FILE *listFp, *dbFp, *nameKeyFp, *tmpFp;
    char line[MXLINELEN];
...
TitleID processTriviaList(struct titleIndexRec *titles, TitleID *titleCount, int listId)
    ...
    while (notdone && fgets(line, MXLINELEN, listFp) != NULL)
        ...
        else if (strcmp(line, triviaDefs[listId].start) == 0)
            inheader = TRUE;
...
TitleID processPlotList(struct titleIndexRec *titles, TitleID *titleCount)
    ...
    while (fgets(line, MXLINELEN, listFp) != NULL)
        ...
        else if (strncmp(line, "PLOT SUMMARIES", 14) == 0)
            {
            inplot = TRUE;
            (void)fprintf(dbFp, "%s", line);
            }
...
TitleID processOutlineList(struct titleIndexRec *titles, TitleID *titleCount)
    ...
    while (fgets(line, MXLINELEN, listFp) != NULL)
        ...
        else if (strncmp(line, "OUTLINES LIST", 13) == 0)
            {
            inplot = TRUE;
            (void)fprintf(dbFp, "%s", line);
            }
...
NameID processBiographiesList(NameID *nameCount)
    ...
    while (fgets(line, MXLINELEN, listFp) != NULL)
        ...
        else if (strncmp(line, "BIOGRAPHY LIST", 14) == 0)
            {
            inbio = TRUE;
            (void)fprintf(dbFp, "%s", line);
            }
...
int mrrDataSort(struct mrrData *m1, struct mrrData *m2) { return (m1->titleKey - m2->titleKey); }
...
TitleID processMovieRatings(struct titleIndexRec *titles, TitleID *titleCount)
    ...
    (void)qsort((void *)ratingsReport, (size_t)count, sizeof(struct mrrData),
                (int (*)(const void *, const void *))mrrDataSort);
...
int voteDataSort(struct voteData *v1, struct voteData *v2) { return (v1->titleKey - v2->titleKey); }
...
TitleID processVotesList(struct titleIndexRec *titles, TitleID *titleCount)
    ...
    (void)qsort((void *)votes, (size_t)count, sizeof(struct voteData),
                (int (*)(const void *, const void *))voteDataSort);
...
int akaIndexDataSort(struct akaData *a1, struct akaData *a2) { return (a1->aka - a2->aka); }
...
TitleID processAkaList(struct titleIndexRec *titles, TitleID *titleCount, AttributeID *attrCount, char *akaFile)
    ...
    while (!enddata && fgets(line, MXLINELEN, listFp) != NULL)
        if (inaka)
            ...
            if (line[0] == '\n')
                ...
                if (sortedTo == 0)
                    count++;
                else if (bsearch((void *)&(aka[count]), (void *)aka, (size_t)sortedTo, sizeof(struct akaData),
                                 (int (*)(const void *, const void *))akaDataSort) == NULL)
                    count++;
                if (count >= akaIndexSize)
                    {
                    struct akaData *akaIndex = aka;
    ...
    (void)qsort((void *)titles, (size_t)*titleCount, sizeof(struct titleIndexRec),
                (int (*)(const void *, const void *))titleIndexRecSort);
    /* sanity check */
    if (1)
        {
    ...
    (void)qsort((void *)aka, (size_t)count, sizeof(struct akaData),
                (int (*)(const void *, const void *))akaIndexDataSort);
...
int akaNameIndexDataSort(struct akaNameData *a1, struct akaNameData *a2) { return (a1->aka - a2->aka); }
...
NameID processAkaNamesList(NameID *nameCount)
    ...
    (void)qsort((void *)naka, (size_t)count, sizeof(struct akaNameData),
                (int (*)(const void *, const void *))akaNameDataAlphaSort);
    ...
    (void)qsort((void *)naka, (size_t)count, sizeof(struct akaNameData),
                (int (*)(const void *, const void *))akaNameDataSort);
    ...
    (void)qsort((void *)naka, (size_t)count, sizeof(struct akaNameData),
                (int (*)(const void *, const void *))akaNameIndexDataSort);
...
TitleID processTitleInfoList(struct titleIndexRec *titles, TitleID *titleCount, AttributeID *attrCount, int listId)
    ...
    (void)qsort((void *)titlesIndex, (size_t)idxCount, sizeof(struct titleKeyOffset),
                (int (*)(const void *, const void *))titleKeyOffsetSort);
...
TitleID processBusinessList(struct titleIndexRec *titles, TitleID *titleCount)
    ...
    while (fgets(line, MXLINELEN, listFp) != NULL)
        ...
        else if (strncmp(line, "BUSINESS LIST", 13) == 0)
            {
            inLit = TRUE;
            (void)fprintf(dbFp, "%s", line);
            }
...
TitleID processLiteratureList(struct titleIndexRec *titles, TitleID *titleCount)
    ...
    while (fgets(line, MXLINELEN, listFp) != NULL)
        ...
        else if (strncmp(line, "LITERATURE LIST", 14) == 0)
            {
            inLit = TRUE;
            (void)fprintf(dbFp, "%s", line);
            }
...
int compCastSort(struct titleKeyOffset *t1, struct titleKeyOffset *t2) { return (t1->titleKey - t2->titleKey); }
...
TitleID processCompleteCastList(struct titleIndexRec *titles, TitleID *titleCount)
    ...
    (void)qsort((void *)list, (size_t)count, sizeof(struct compCastRec),
                (int (*)(const void *, const void *))compCastSort);
...
TitleID processMovieLinksList(struct titleIndexRec *titles, TitleID *titleCount)
                ...
                for (i = 0; i < NO_OF_LINK_TYPES - 1; i++)
                    if (strncmp(line + 3, movieLinkDefs[i].string, movieLinkDefs[i].length) == 0)
                        ...
                        if ((ptr = strrchr(line, ')')) != NULL)
                            {
                            *ptr = '\0';
                            list[count].toTitleKey =
                                titleKeyLookup(line + 4 + movieLinkDefs[i].length, titles, titleCount);
    ...
    (void)qsort((void *)list, (size_t)count, sizeof(struct movieLinkDbRec),
                (int (*)(const void *, const void *))movieLinkRecSort);
    for (i = 0; i < count; i++)
        {
